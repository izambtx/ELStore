...
class UsersModel extends Model
{
    ...
    protected $allowedFields = ['id', 'username', 'fullname', 'email', 'alamat', 'noHP', 'password_hash'];

    public function getPetugas($id = false)
    {
        if ($id == false) {
            return $this->join('auth_groups_users', 'auth_groups_users.user_id = users.id')->where('auth_groups_users.group_id', '3')->orderBy('fullname')->findAll();
        }

        return $this->join('auth_groups_users', 'auth_groups_users.user_id = users.id')->where('auth_groups_users.group_id', '3')->where(['id' => $id])->first();
    }
    ...
    public function getPembuat($id = false)
    {
        if ($id == false) {
            return $this->join('auth_groups_users', 'auth_groups_users.user_id = users.id')->where('auth_groups_users.group_id', 2)->orderBy('fullname')->findAll();
        }

        return $this->join('auth_groups_users', 'auth_groups_users.user_id = users.id')->where('auth_groups_users.group_id', 2)->where(['id' => $id])->first();
    }
