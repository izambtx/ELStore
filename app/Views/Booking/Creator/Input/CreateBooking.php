        ...
        <div class="col-lg-12">
            <div class="card text-gray-900 shadow">
                <div class="card-body">
                    ...
                    <form method="post" action="/Form-Save-Booking" enctype="multipart/form-data">
                        <?php csrf_field(); ?>

                        <div class="row">
                            <div class="col">
                                <input type="text" class="<?php if (session('validation.nama')) : ?>is-invalid<?php endif ?> form-control mt-4 rounded-sm" placeholder="Nama Lengkap" name="nama" id="nama" value="<?= old('nama') ? old('nama') : '' ?>">
                                ...
                                <select id="petugas" name="petugas" class="<?php if (session('validation.petugas')) : ?>is-invalid<?php endif ?> form-control mt-4 rounded-sm">
                                    ...
                                    <?php foreach ($petugas as $p) : ?>
                                        <option value="<?= $p['id'];  ?>" <?= old('petugas') == $p['id'] ? 'selected' : '' ?>><?= $p['fullname'];  ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <div class="invalid-feedback">
                                ...
                                <select id="layanan2" name="layanan2" class="<?php if (session('validation.layanan2')) : ?>is-invalid<?php endif ?> form-control mt-4 rounded-sm">
                                    ...
                                    <?php foreach ($layanan as $l) : ?>
                                        <option value="<?= $l['id'];  ?>" <?= old('layanan2') == $l['id'] ? 'selected' : '' ?>><?= $l['nama_layanan'];  ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <div class="invalid-feedback">
                            ...
                            <div class="col">
                                <input type="datetime-local" class="<?php if (session('validation.tanggal')) : ?>is-invalid<?php endif ?> form-control mt-4 rounded-sm" id="tanggal" name="tanggal" value="<?= old('tanggal') ? old('tanggal') : '' ?>">
                                <div class="invalid-feedback">
                                ...
                                <select id="layanan1" name="layanan1" class="<?php if (session('validation.layanan1')) : ?>is-invalid<?php endif ?> form-control mt-4 rounded-sm">
                                    ...
                                    <?php foreach ($layanan as $l) : ?>
                                        <option value="<?= $l['id'];  ?>" <?= old('layanan1') == $l['id'] ? 'selected' : '' ?>><?= $l['nama_layanan'];  ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <div class="invalid-feedback">
                                ...
                                <select id="layanan3" name="layanan3" class="<?php if (session('validation.layanan3')) : ?>is-invalid<?php endif ?> form-control mt-4 rounded-sm">
                                    ...
                                    <?php foreach ($layanan as $l) : ?>
                                        <option value="<?= $l['id'];  ?>" <?= old('layanan3') == $l['id'] ? 'selected' : '' ?>><?= $l['nama_layanan'];  ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <div class="invalid-feedback">
                        ...
                        <div class="row">
                            <div class="col">
                                <textarea class="form-control mt-4 rounded-sm" placeholder="Catatan Keperluan (Optional)" id="catatan" name="catatan"><?= old('catatan') ? old('catatan') : '' ?></textarea>
